...
AddIn xai_option_cgf(
	...
	.Arguments({
		Arg(XLL_DOUBLE, L"s", L"is the argument of the cumulant generating function."),
		Arg(XLL_HANDLEX, L"m", L"is the handle to a model."),
		})
		.Category(CATEGORY)
	.FunctionHelp(L"Return cumulant generating function kappa(s) of the model.")
);
...
AddIn xai_option_bsm_put(
	...
	.Arguments({
		Arg(XLL_DOUBLE, L"r", L"is the continuously compounded risk-free rate."),
		...
		.Category(CATEGORY)
	.FunctionHelp(L"Return price of a European put option under BSM parameters using the model.")
);
